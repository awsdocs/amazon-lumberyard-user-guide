...
Lumberyard uses an audio translation layer \(ATL\) to interface between Lumberyard and third\-party audio middleware, so you can change your audio implementation without affecting the game logic\. The game logic interacts with ATL controls, which map to their audio middleware equivalents\. For example, to play a sound, the game executes an ATL trigger, which is mapped to a 'Play' event in the audio middleware\.
...
For more information, see [Audiokinetic Wwise LTX](audio-wwise-using.md)\.
...
+ [Audio Implementations](audio-implementations.md)
+ [Audio Console Variables Commands](audio-console-commands.md)
